...
export default function ProjectHeader({ pTitle, pDescription, type, setShowModal }) {
    ...
    return (
        <div className="flex justify-between items-center flex-col md:flex-row">
            ...
            <div className="flex flex-wrap gap-2 justify-between sm:justify-end w-full md:w-auto">
                <Button size="lg" className="flex-1 rounded-sm hover:bg-gray-800 cursor-pointer"
                    onClick={() => setShowModal(true)}>
                    <Icon size={18} />
                    {type === "create" ? <span className="hidden md:block">Create Project</span> : <span className="hidden md:block">Edit Project</span>}
                </Button>

                {type === "edit" && (
                    <>
                        <Button size="lg" variant="outline" className="flex-1 rounded-sm cursor-pointer"
                            onClick={() => setShowModal(true)}>
                            <PlusSquare size={18} />
